...
- Проксируем все заголовки (кроме `Host`); по опции можно вырезать `Cookie`, чтобы не протекали сессионные куки на внешние сервисы, и добавляем `X-Internal-Secret` для внутренней аутентификации между сервисами.
- Гарантируем наличие `Request-Id`/`X-Request-Id`, чтобы искать запрос по логам в Gateway и бэкендах.
...
| `SERVICE_PROXY_STRIP_COOKIES` | Не пересылать заголовок `Cookie` на upstream | `false` |
| `SERVICE_PROXY_SHARED_SECRET` | Добавить `X-Internal-Secret` во все проксированные запросы | пусто |
...
- `strip_cookies` — опционально вырезает исходный `Cookie` при обращении к микросервисам (уместно, если авторизация происходит только через JWT).
- `shared_secret` — задаёт строку, которая будет передаваться в `X-Internal-Secret` для авторизации внутреннего трафика.
- Конфиг читается однократно и кешируется в памяти процесса, чтобы не делать лишние файловые операции на каждом запросе.
- Базовые URL должны начинаться с `http://` или `https://`; остальные схемы/пустые строки игнорируются и не будут проксироваться.
